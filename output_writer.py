...
Single channel  → calibration_ch####.txt
Multi channel   → calibration_log.txt  +  calibration_coeffs.txt

Nonlinear model:  E = P0·(P1^Q)^P2 + P3·Q − P0
Linear model:     E = P0 + P1·Q
"""
...
class OutputWriter:
    ...
    def write_single(cls, result: FitResult, out_path: str,
        ...
        with open(out_path, "w") as f:
            ...
            f.write("#\n")
            f.write(f"# {'ADC_position':>14s}  "
                    f"{'Energy_keV':>12s}  {'Residual_keV':>14s}\n")
    ...
    @classmethod
    def write_log(cls, results: dict[int, FitResult], out_path: str,
                   source_file: str = ""):
        ...
        with open(out_path, "w") as f:
            ...
            for ch_id in sorted(results.keys()):
                ...
                f.write(f"# Chi2/NDF: {r.chi2_ndf:.6f}  "
                        f"(Chi2={r.chi2:.3f}, NDF={r.ndf})\n#\n")
                f.write("# Parameters:\n")
                for name, val, unc in zip(r.param_names,
                                           r.params, r.uncertainties):
                    f.write(f"#   {name} = {val:+.8e}  ±  {unc:.3e}\n")
                f.write("#\n")
                f.write(f"# {'ADC_position':>14s}  "
                        f"{'Energy_keV':>12s}  {'Residual_keV':>14s}\n")
                for adc, eng, res in zip(r.adc_points,
                                          r.energy_points, r.residuals):
                    f.write(f"  {adc:>14.4f}  {eng:>12.3f}  "
                            f"{res:>+14.5f}\n")

    # ------------------------------------------------------------------ #
    # Multi channel — compact coefficients
    # ------------------------------------------------------------------ #

    @classmethod
    def write_coeffs(cls, results: dict[int, FitResult], out_path: str,
                      source_file: str = ""):
        good = [r for r in results.values() if not r.bad_channel]
    ...
    @classmethod
    def write_all(cls, results: dict[int, FitResult],
                   output_dir: str, source_file: str = "",
                   prefix: str = "calibration") -> list[str]:
        os.makedirs(output_dir, exist_ok=True)
        if len(results) == 1:
            ch_id = next(iter(results))
            path  = os.path.join(output_dir,
                                  f"{prefix}_ch{ch_id:04d}.txt")
